...
#define TAG "GATTS_AND_WIFI_DEMO"
...
#define TEST_DEVICE_NAME "ESP_GATTS_AND_WIFI_DEMO_V5_2_6"
...
/*EXT_RAM_BSS_ATTR */ TaskHandle_t ble_channel_task = NULL;
static StaticTask_t ble_channel_static_task;
#define BLE_CHANNEL_TASK_STACK_SIZE 2304
static const size_t ble_channel_task_priority = 5;
/*EXT_RAM_BSS_ATTR */ static StackType_t ble_channel_task_stack[BLE_CHANNEL_TASK_STACK_SIZE];

void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param);
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param);
void print_task_remaining_memory();

void print_task_remaining_memory()
{
    UBaseType_t highWaterMark = uxTaskGetStackHighWaterMark(NULL); // NULL means the calling task
    TaskHandle_t currentTaskHandle = xTaskGetCurrentTaskHandle();
    const char *currentTaskName = pcTaskGetName(currentTaskHandle);
    ESP_LOGD(TAG, "Remaining memory for task '%s': %u bytes", currentTaskName, highWaterMark * 4);
}

static void ble_channel_switch_task()
{
    esp_err_t ret __attribute__((unused)) = ESP_OK; // used by ESP_GOTO_ON_ERROR
    while (true)
    {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        ESP_LOGI(TAG, "Optimizing BLE channels...");

        wifi_ap_record_t ap_info;
        ESP_GOTO_ON_ERROR(esp_wifi_sta_get_ap_info(&ap_info), err, TAG, "Failed to obtain AP info");
        ESP_LOGI(TAG, "WiFi connected to channel: %d", ap_info.primary);

        // Find the best BLE advertising channels based on the WiFi channel
        adv_params.channel_map = map_wifi_to_ble_adv_channels(ap_info.primary);
        ESP_GOTO_ON_ERROR(esp_ble_gap_stop_advertising(), err, TAG, "Failed to stop advertising");
        ESP_GOTO_ON_ERROR(esp_ble_gap_start_advertising(&adv_params), err, TAG, "Failed to start advertising");
        log_ble_adv_channels(TAG, adv_params.channel_map, "BLE advertising channels updated:");

        // Find the best BLE data channels based on the WiFi channel
        esp_gap_ble_channels channels = {0};
        map_wifi_to_ble_data_channels(ap_info.primary, channels);
        ESP_GOTO_ON_ERROR(esp_gap_ble_set_channels(channels), err, TAG, "Failed to set BLE data channels");
        log_ble_data_channels(TAG, channels, "BLE data channels updated:");

    err:
        print_task_remaining_memory();
    }
}

static void create_ble_task()
{
    if (ble_channel_task == NULL)
    {
        ble_channel_task = xTaskCreateStatic(
            ble_channel_switch_task,
            "ble_channel_switch",
            BLE_CHANNEL_TASK_STACK_SIZE,
            NULL,
            ble_channel_task_priority,
            ble_channel_task_stack,
            &ble_channel_static_task);
    }
}
...
void app_main(void)
{
    ...
    {
        ESP_LOGE(TAG, "Failed to set CPU frequency");
        return;
    }
